<script lang="ts">
  ...
  type SocialLink = {
    ...
    acf: {
      platform_name: string; // English platform name
      'بستر': string; // Persian platform name
      url: string;
    };
  };

  const socialLinks = writable<SocialLink[]>([]);
  ...
  onMount( async () => {
    ...
    // Fetch profile and social links from API
    try {
      ...
      if (profileData.length > 0) {
        ...
        if (profile.acf.social_links?.length) {
          ...
          socialLinks.set(await res.json());
        }
      }
    } catch (error) {
...
<footer class="flex flex-col justify-center bg-gray-900 text-white py-8 sm:py-12 px-4 sm:px-8 md:px-16" dir={$currentLanguage === 'fa' ? 'rtl' : 'ltr'}>
  <div class="max-w-6xl mx-auto">
    ...
    <div class="links-grid flex flex-wrap justify-center gap-2 sm:gap-4 mb-6 sm:mb-8">
      {#if $socialLinks.length}
        {#each $socialLinks as link}
          <FooterLink {link} />
        {/each}
      {:else}
        <a href="mailto:matin.arno4646@gmail.com" class="text-sm sm:!text-lg inline-flex items-center px-4 sm:px-6 py-2 sm:py-3 bg-blue-600 text-white rounded-lg shadow-lg footer-link footer-animate-init"
          aria-hidden="true"
    ...
    <!-- Copyright -->
    <div class="copyright-div text-center lg:text-left pt-6 sm:pt-8 border-t border-gray-700">
